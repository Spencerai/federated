...
def _has_unique_names(comp):
  """Checks that each variable of `comp` is specified at most once."""
  names = set()
  unique = [True]
...
class TransformationsTest(parameterized.TestCase):
  ...
  def test_rename_references_and_callers_single_level_block(self):
    ...
    renamed = transformations.rename_references_and_callers(block)
    self.assertEqual(
        renamed.tff_repr,
        '(let {0}1=data,{0}2={0}1,{0}3={0}2 in {0}3)'.format(RENAME_PREFIX))

  def test_rename_references_and_callers_nested_blocks(self):
    x_ref = computation_building_blocks.Reference('x', tf.int32)
    input1 = computation_building_blocks.Data('input1', tf.int32)
    ...
    renamed = transformations.rename_references_and_callers(block2)
  ...
  def test_rename_references_and_callers_nested_lambdas(self):
    ...
    renamed = transformations.rename_references_and_callers(second_level_call)
    self.assertTrue(_has_unique_names(renamed))
    self.assertEqual(
        renamed.tff_repr,
        '({0}1 -> {0}1)(({0}2 -> {0}2)(test))'.format(RENAME_PREFIX))

  def test_rename_references_and_callers_block_lambda_block_lambda(self):
    x_ref = computation_building_blocks.Reference('x', tf.int32)
    ...
    last_block = computation_building_blocks.Block([('x', final_input),
                                                    ('x', x_ref)], second_call)
    renamed = transformations.rename_references_and_callers(last_block)
    self.assertEqual(
        last_block.tff_repr,
  ...
  def test_rename_references_and_callers_blocks_nested_inside_of_locals(self):
    x_data = computation_building_blocks.Data('x', tf.int32)
    data = computation_building_blocks.Data('data', tf.int32)
    ...
    renamed = transformations.rename_references_and_callers(
